...
- Set Plutonium competitive DVARs on the patch level
- Fridge can now be set with a specific attachment using original `+<attachment>` syntax
- Permaperks that are removed on a specific map now have their progress stats reset regardless whether players already have them
- Optimize allocations

# Version 4.11

- The box tracker print can now be triggered via chat `box` message even when it's disabled
- The box tracker average calculation was optimized and made to work similar to standalone box trackers
- Added gamelog based integration for external box tracking
...
- The box tracker print is now disabled by default, details in the readme
- The box tracker print is now delayed and shows after the gun is either picked up or times out in the box

# Version 4.9

- Fix duplicating watermarks

# Version 4.8

- Fix `backspeed` message not clamping the input when a single value is used

# Version 4.7

*This patch is following a vote from February 2026*

- Fix watermarks being able to draw on top of each other
- Fix box location mover not properly integrating with the internal box location cycle
- Box location command now moves the box instantly
- Disabled tank command on maps other than Origins
- Increase the duration the message with box stats is staying on the screen
- Rework aliases for chat commands (check readme for current aliases)
- Allow players to pick up the Hell's Redeemer after reconnecting
- Bump deprecated Pluto version
- Key patch status can now be checked by using `key` chat message
...
- Add support for animated camos on Buried and Origins (4516+)
- Add Ghost (afterlife) to the possible options of viewmodel swap

# Version 4.4

- Added Purist mode to enable playing No Jug games without persistent jug (minijug) for Plutonium
...
- Backspeed can now be set via `bs` chat message on Plutonium
- Fix custom dvars on Redacted & Ancient

# Version 4.3

- Added integration for strat tester mods for Modern Plutonium (4516+)
